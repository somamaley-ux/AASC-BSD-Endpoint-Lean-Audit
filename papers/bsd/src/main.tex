...
\newcommand{\BridgeImgExclLoc}{\mathrm{BridgeImgExcl}^{\mathrm{loc}}_{\BSD}}
\newcommand{\ThmRankDisc}{\mathrm{ThmRankDisc}_{\BSD}}
\newcommand{\RankEq}{\mathrm{RankEq}_{\BSD}}
\newcommand{\ReductioBSD}{\mathrm{ReductioCountercase}_{\BSD}}
\newcommand{\LocalCountercaseBSD}{\mathrm{LocalCountercase}_{\BSD}}
\newcommand{\EndpointCounterforceBSD}{\mathrm{EndpointCounterforce}_{\BSD}}
\newcommand{\BsdRankBridge}{B^{\mathrm{rank}}_{\BSD}}
\newcommand{\BsdRankBridgeComp}{B^{\mathrm{rank,comp}}_{\BSD}}
...
The endpoint closeout also has a local exact-complement form. In the fixed curvewise rank endpoint proof, the sole local assumption
\[
[\RankMismatch(E)]_i
\]
may be opened as the live countercase against rank equality. The local route is
\[
\begin{aligned}
[\RankMismatch(E)]_i
&\Rightarrow \LocalCountercaseBSD(E)\Rightarrow \EndpointCounterforceBSD(E)\\
&\Rightarrow \BridgeImgExclLoc(E)\Rightarrow \mathrm{IndependentBSDRankDiscriminator}(E)\Rightarrow \bot.
\end{aligned}
\]
The annotation records a proof role, not an extra object-level premise. The contradiction therefore discharges the original mismatch assumption and yields rank equality on the fixed carrier.

\subsection*{No premature negative-resolution promotion}
The proof does not read raw $\RankMismatch(E)$ as endpoint governance. The mismatch branch first appears as ordinary analytic and arithmetic content: distinct analytic and Mordell-Weil rank values. It is then identified with standard mismatch normal form and bridge-image exclusion on the fixed carrier.

Only when that bridge-image exclusion is used either as official negative endpoint resolution or as the live exact-complement countercase inside the rank endpoint proof does it enter endpoint-governance discipline. This is the BSD analogue of exact-complement discipline in the companion endpoint papers.

\subsection*{Audit and truth-boundary posture}
The companion Lean 4 archive is a reproducibility and theorem-routing audit for this proof spine, not a replacement for the mathematical manuscript and not a full arithmetic-geometry library. It audits the fixed-carrier endpoint route: kernel-governed endpoint use, rank-mismatch occupation, bridge-image exclusion, theorem-level endpoint-status governance, independent-discriminator exclusion, rank-mismatch impossibility, pointwise rank equality, official rank-endpoint correspondence, and the refined formula bridge/factor-standing boundary. The archive also records a truth-boundary ledger separating closed AASC proof-spine claims from semantic carrier abstraction, arithmetic background standing, role-compression reference material, and manuscript release status. The mathematical proof route below is therefore the source of the argument; Lean records the proof-spine audit surface and its reproducibility boundary.
...
\section{Local reductio engine and rank endpoint closeout}

The global official-negative-resolution exclusion remains available as Corollary~\ref{cor:rank-mismatch-excluded}. The endpoint closeout below uses the local exact-complement route so that a temporary mismatch assumption is not promoted to a global negative endpoint outcome.

\begin{definition}[Rank equality target]\label{def:rank-eq-target}
For the fixed curve $E/\Q$, write
\[
\RankEq(E)
\Longleftrightarrow
\ord_{s=1}L(E,s)=\rank E(\Q).
...
\begin{definition}[Exact-complement rank-mismatch annotation]\label{def:rank-reductio-annotation}
\[
\ReductioBSD(\RankMismatch(E);\RankEq(E))
\]
is the proof-act annotation generated when the sole local assumption $[\RankMismatch(E)]_i$ is opened as the live exact-complement countercase against the rank equality target on the same fixed BSD carrier. The annotation records the role of the assumption; it is not a second object-level hypothesis.
\end{definition}

\begin{theorem}[Annotation is not a premise]\label{thm:rank-annotation-not-premise}
The exact-complement annotation does not transform the discharged assumption into
\[
\RankMismatch(E)\wedge \ReductioBSD(\RankMismatch(E);\RankEq(E)).
\]
The only object-level assumption opened in the local subproof is $\RankMismatch(E)$.
\end{theorem}

\begin{proof}
A reductio annotation records how the assumption is being used: it is the live complement of the endpoint target in the fixed proof context. It does not add mathematical content to the assumption. Thus a contradiction in the annotated subproof discharges $\RankMismatch(E)$ itself, not a strengthened conjunction.
\end{proof}

\begin{definition}[Local BSD mismatch countercase]
The shorthand $\LocalCountercaseBSD(E)$ means that $\RankMismatch(E)$ is being used inside the rank endpoint proof as the live exact-complement countercase against $\RankEq(E)$ on the fixed carrier.
\end{definition}

\begin{theorem}[Exact-complement mismatch has local countercase standing]\label{thm:rank-mismatch-local-countercase}
On the adequate carrier, the exact-complement annotation supplies local countercase standing.
\end{theorem}

\begin{proof}
On the adequate carrier, analytic rank and Mordell-Weil rank are unique integer-valued roles. Their equality is the pointwise rank target, while $\RankMismatch(E)$ asserts that the fixed analytic and arithmetic values are distinct. Opening this exact complement against $\RankEq(E)$ gives temporary local countercase standing. It does not assert global negative endpoint resolution.
\end{proof}

\begin{theorem}[Local countercase has endpoint counterforce]\label{thm:local-countercase-counterforce}
\[
\LocalCountercaseBSD(E)
\Rightarrow
\EndpointCounterforceBSD(E).
\]
\end{theorem}

\begin{proof}
The local countercase is introduced precisely as the branch whose survival would block rank equality on the fixed carrier. That endpoint-facing role is endpoint counterforce. The conclusion is local proof discipline, not theoremhood of the negative branch.
\end{proof}

\begin{definition}[Local bridge-image exclusion]
$\BridgeImgExclLoc(E)$ means bridge-image exclusion used inside the exact-complement reductio subproof as the endpoint-defeating negative branch on the fixed BSD rank carrier.
\end{definition}

\begin{theorem}[Local counterforce projects to local bridge-image exclusion]\label{thm:local-counterforce-bridge-exclusion}
On the adequate carrier, endpoint counterforce plus the live mismatch assumption supplies local bridge-image exclusion.
\end{theorem}

\begin{proof}
The correspondence bridge identifies $\RankMismatch(E)$ with $\BridgeImgExcl(E)$ on the adequate carrier by Theorem~\ref{thm:mismatch-bridge-exclusion}. Endpoint counterforce says this bridge-image exclusion is not merely descriptive or support-level content; it is being used locally to block the rank endpoint. That is precisely local bridge-image exclusion.
\end{proof}

\begin{theorem}[Local bridge-image exclusion induces the independent discriminator]\label{thm:local-bridge-exclusion-independent}
\[
\BridgeImgExclLoc(E)
\Rightarrow
\mathrm{IndependentBSDRankDiscriminator}(E).
...
\begin{proof}
Local bridge-image exclusion is endpoint-facing bridge exclusion on the same fixed carrier. It is not analytic standing alone, not Mordell-Weil readout alone, and not bridge completion. It is not support-only or bookkeeping, because it is used as the live countercase whose survival blocks rank equality. It is not carrier shift, because the local reductio is conducted on $C_{\BSD}(E)$. It is not a lawful coequal target role by Theorem~\ref{thm:mismatch-not-coequal}. After these roles are removed, the remaining local endpoint-status work is independent same-domain rank discrimination.
\end{proof}

\begin{corollary}[Local rank-mismatch countercase contradiction]\label{cor:local-rank-mismatch-contradiction}
Under official fixed-carrier BSD endpoint evaluation, the live assumption $\RankMismatch(E)$ with exact-complement annotation $\ReductioBSD(\RankMismatch(E);\RankEq(E))$ yields contradiction.
\end{corollary}

\begin{proof}
The annotation gives local countercase standing by Theorem~\ref{thm:rank-mismatch-local-countercase}, and local countercase standing gives endpoint counterforce by Theorem~\ref{thm:local-countercase-counterforce}. With the live mismatch assumption, Theorem~\ref{thm:local-counterforce-bridge-exclusion} gives $\BridgeImgExclLoc(E)$. Theorem~\ref{thm:local-bridge-exclusion-independent} gives an independent BSD rank discriminator. Official endpoint use instantiates $\Kern_{\BSD}(E)$ by Theorem~\ref{thm:endpoint-instantiates-kernel}; Theorem~\ref{thm:no-independent-rank-disc} excludes the independent discriminator. Contradiction follows.
\end{proof}

\begin{theorem}[Annotation discharge gives rank equality]\label{thm:annotation-discharge-rank-eq}
For a fixed $E/\Q$, if the annotated exact-complement subproof from $[\RankMismatch(E)]_i$ yields contradiction, then the discharged result is $\neg\RankMismatch(E)$. Under $\mathrm{BSDCarrierAdeq}(E)$, $\neg\RankMismatch(E)$ implies $\RankEq(E)$.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:rank-annotation-not-premise}, the annotation is not an added object-level hypothesis. Classical reductio therefore discharges the sole opened assumption $\RankMismatch(E)$ and yields $\neg\RankMismatch(E)$. Under carrier adequacy, Lemma~\ref{lem:rank-total-unique} gives unique integers $n,m$ with $\Van(E,n)$ and $\RankRead(E,m)$. If $n\neq m$, then $\RankMismatch(E)$ holds, contrary to $\neg\RankMismatch(E)$. Hence $n=m$, which is exactly $\RankEq(E)$ by the definitions of analytic standing and Mordell-Weil rank readout.
\end{proof}
...
\begin{proof}
Fix an arbitrary elliptic curve $E/\Q$ and evaluate the ordinary rank-BSD target on its fixed carrier. By Theorem~\ref{thm:universal-binds-pointwise}, this target-fixation act supplies $\mathrm{OfficialBSDEndpointUse}(E)$ and $\mathrm{BSDCarrierInstantiated}(E)$ without assuming rank equality. By Theorem~\ref{thm:carrier-adequate}, the instantiated carrier is adequate, and by Theorem~\ref{thm:endpoint-instantiates-kernel}, the fixed-carrier regime is kernel-governed by necessity.

Open the exact-complement assumption $[\RankMismatch(E)]_i$ against the target $\RankEq(E)$, with annotation $\ReductioBSD(\RankMismatch(E);\RankEq(E))$. Corollary~\ref{cor:local-rank-mismatch-contradiction} gives contradiction in that annotated subproof. By Theorem~\ref{thm:annotation-discharge-rank-eq}, the discharged result is $\RankEq(E)$, namely
\[
\ord_{s=1}L(E,s)=\rank E(\Q).
\]
...
The endpoint proof distinguishes the bridge object, bridge admissibility, and bridge completion. This prevents circularity: the route and admissibility conditions do not contain rank equality by definition. Rank equality appears only when the bridge is completed or when all endpoint-resolving mismatch occupations are excluded.

\begin{definition}[Rank bridge object]\label{def:rank-bridge-object}
For a fixed elliptic curve $E/\Q$ and integer $n\in\Z_{\ge0}$, the rank bridge object
\[
\BsdRankBridge(E,n)
\]
records a lawful bridge from analytic vanishing-order standing to Mordell-Weil rank readout on the same fixed BSD carrier. It requires the following clauses.
\begin{enumerate}[label=\textbf{B\arabic*.},leftmargin=3.2em]
\item \textbf{Same-curve preservation.} The bridge preserves the same elliptic curve $E/\Q$, not an isogenous surrogate, twist, family member, model analogue, or neighboring curve unless a lawful carrier bridge returns to $E$.
\item \textbf{Analytic-carrier preservation.} The bridge preserves the same $L(E,s)$, the same distinguished point $s=1$, and the same analytic standing predicate $\Van(E,n)\Longleftrightarrow \ord_{s=1}L(E,s)=n$.
\item \textbf{Arithmetic-carrier preservation.} The bridge preserves the same Mordell-Weil group $E(\Q)$, its torsion quotient, and the same free-rank readout predicate $\RankRead(E,n)\Longleftrightarrow \rank E(\Q)=n$.
\item \textbf{Common integer-value preservation.} The bridge uses the same integer $n$ on both sides. A route from $\Van(E,n)$ to $\RankRead(E,m)$ with $n\neq m$ is not bridge completion; it is bridge-image exclusion.
\item \textbf{Bridge-certification discipline.} Descent, Selmer, Heegner, Iwasawa, regulator, Sha, numerical, or analytic material counts as bridge completion only when it supplies theorem-bearing support for common occupation of the same rank value on the fixed carrier.
\item \textbf{UEAP report preservation.} The bridge does not report analytic standing as arithmetic readout, arithmetic readout as analytic standing, or bridge support as bridge completion unless the corresponding endpoint role is supplied.
\item \textbf{ATS anchor/tensor/skin preservation.} The bridge preserves the same curve, analytic carrier, Mordell-Weil carrier, and point $s=1$. It treats rank equality and bridge occupation as tensor content, while notation, basis choices, search cutoffs, numerical precision, representative choices, and proof-route labels remain skin unless a bridge role is supplied.
\end{enumerate}
\end{definition}
...
B^{\slot}_{\BSD}(E,n),
\]
the declared transition position between $\Van(E,n)$ and $\RankRead(E,n)$ on the fixed carrier. The bridge object $\BsdRankBridge(E,n)$ records the same slot together with the carrier-preservation, certification, UEAP, and ATS conditions needed to audit completion.
\end{definition}

\begin{definition}[Rank bridge admissibility]
$B^{\adm}_{\BSD}(E,n)$ holds when a proposed transition preserves the same elliptic curve, the same analytic carrier, the same Mordell-Weil carrier, the same integer value, and the same readout slot, while excluding surrogate substitution, carrier transfer, post-selection repair, and second-classifier governance. Equivalently, it is the admissibility layer of $\BsdRankBridge(E,n)$.
\end{definition}

\begin{definition}[Bridge-complete rank object]\label{def:rank-bridge-complete-object}
The bridge-complete rank object
\[
\BsdRankBridgeComp(E,n)
\]
means $\BsdRankBridge(E,n)$ together with theorem-bearing common occupation of the rank value:
\[
\Van(E,n)\wedge \RankRead(E,n).
\]
This is the completed form of the older bridge-completion notation $B^{\comp}_{\BSD}(E,n)$.
\end{definition}

\begin{proposition}[Rank bridge object is non-circular]\label{prop:rank-bridge-noncircular}
The bridge object $\BsdRankBridge(E,n)$ does not contain rank equality by definition. It fixes the same-carrier bridge route and the audit conditions under which bridge completion may be reported. Rank equality appears only under bridge completion or after endpoint-resolving mismatch branches are excluded.
\end{proposition}

\begin{proof}
If the bridge object contained $\RankRead(E,n)$ as a conclusion of analytic standing alone, BSD would be definitional. If admissibility contained rank equality, no incomplete descent, Selmer, Heegner, Iwasawa, or analytic route could be audited as bridge support. The object fixes the curve, analytic carrier, arithmetic carrier, common integer-value interface, certification burden, and report discipline; it does not assert that the bridge is already occupied.
\end{proof}

\begin{theorem}[Bridge-complete rank object yields common rank value]\label{thm:bridge-complete-yields-rank-value}
For every $E/\Q$ and $n\in\Z_{\ge0}$,
\[
\BsdRankBridgeComp(E,n)
\Rightarrow
\ord_{s=1}L(E,s)=\rank E(\Q)=n.
\]
\end{theorem}

\begin{proof}
By definition, $\BsdRankBridgeComp(E,n)$ includes $\Van(E,n)$ and $\RankRead(E,n)$ on the same fixed carrier. The carrier exactness clauses identify these with $\ord_{s=1}L(E,s)=n$ and $\rank E(\Q)=n$. Hence the analytic and arithmetic rank values coincide at $n$.
\end{proof}

\begin{theorem}[Fixed-value bridge failure is bridge-image exclusion]\label{thm:fixed-value-bridge-failure}
Under $\mathrm{BSDCarrierAdeq}(E)$,
\[
\Van(E,n)\wedge \RankRead(E,m)\wedge n\neq m
\Rightarrow
\BridgeImgExcl(E).
\]
\end{theorem}
...
\begin{proof}
The bridge object requires a common integer value for completion. If analytic standing occupies $n$ while Mordell-Weil readout occupies $m\neq n$, the two fixed endpoint images fail common bridge occupation. By Definition of $\BridgeImgExcl(E)$, this is bridge-image exclusion on the same carrier.
\end{proof}
...
Local reductio route & $[\RankMismatch(E)]_i$ is used as the live exact-complement countercase against rank equality. & Annotation-discharge discipline; local bridge-image exclusion induces the same forbidden discriminator without global negative-resolution promotion.\\
\bottomrule
\end{longtable}

\begin{theorem}[Rank mismatch route-locus exhaustion]
Every same-carrier endpoint-resolving rank mismatch route, including the local exact-complement route, enters one of the loci above. No additional same-domain locus remains.
\end{theorem}

\begin{proof}
An endpoint-resolving mismatch must change status, substitute a readout, declare bridge vacancy, transfer carrier standing, repair temporally, assemble locally, select by a non-bridge criterion, introduce a second endpoint classifier, or operate as the live local exact-complement countercase. If it does none of these, it does no endpoint work and cannot resolve BSD. Each active coordinate is blocked by the corresponding kernel or report-discipline result recorded in the table.
\end{proof}
...
\item The rank bridge object $\BsdRankBridge(E,n)$ as a non-circular carrier/route object.
\item Local exact-complement reductio annotation as proof-role metadata rather than an added premise.
\item Local mismatch countercase projection to local bridge-image exclusion.
\item Annotation discharge of the original mismatch assumption.
...
Rank bridge object & A clause in $\BsdRankBridge(E,n)$ that smuggles rank equality, changes carrier, or fails to preserve the official BSD rank interface.\\
Bridge-complete readout & A completed bridge object that does not imply a common analytic/arithmetic rank value.\\
Local reductio annotation & A proof that the exact-complement annotation is an extra object-level assumption rather than proof-role metadata.\\
Local countercase projection & A live mismatch countercase that blocks rank equality while not projecting to local bridge-image exclusion.\\
Annotation discharge & A proof that the local contradiction discharges only a strengthened premise rather than $[\RankMismatch(E)]_i$.\\
...
The rank bridge object contains rank equality. & No. $\BsdRankBridge(E,n)$ fixes same-curve, analytic-carrier, arithmetic-carrier, common-value, bridge-certification, UEAP, and ATS discipline. Rank equality appears only under bridge completion or mismatch exclusion.\\
Local reductio assumes official negative resolution. & No. The local assumption is $[\RankMismatch(E)]_i$, used as the exact-complement countercase. It receives local endpoint counterforce, not global official negative endpoint status.\\
Contradiction discharges a strengthened premise. & No. The annotation is proof-role metadata. The only discharged object-level assumption is $\RankMismatch(E)$.\\
...
\item The rank bridge object $\BsdRankBridge(E,n)$ fixes same-carrier bridge conditions without containing rank equality.
\item Bridge-complete rank objects yield a common analytic/arithmetic rank value.
...
\item Exact-complement reductio annotation is proof-role metadata, not an added premise.
\item The local mismatch countercase has endpoint counterforce and projects to local bridge-image exclusion.
\item Local bridge-image exclusion induces the independent rank discriminator and the kernel excludes it.
\item The local contradiction discharges the mismatch assumption itself.
...
Local mismatch assumption & Temporary proof role & Not official negative resolution; used only inside the exact-complement reductio.\\
Local contradiction & Proof-rule closure & Discharges $[\RankMismatch(E)]_i$, not a strengthened premise.\\
Rank bridge object & Carrier/route object & Does not assert rank equality by definition.\\
...
\end{verbatim}
The truth-boundary ledger separates closed Lean proof-spine claims from semantic carrier abstraction, structural AASC role-compression boundaries, external arithmetic standing, and manuscript release status. Its completion anchor is:
\begin{verbatim}
...
The focused BSD checks report no project-level axioms for the audited BSD closeout anchors on the active proof surface. The pre-Lean formalization map remains outside the active import graph and may contain signature-map declarations; the aggregate audit checks that this map is not imported by the executable audit surface before parsing it separately. This separation prevents signature-map material from being counted as an axiom-free proof file.

The explicit rank bridge object and the local exact-complement reductio annotation are manuscript-level defensive refinements of the same proof spine already represented in the Lean audit surface: rank mismatch, standard mismatch normal form, bridge-image exclusion, negative endpoint use, theorem-level discriminator, independent rank discriminator, no-independent-discriminator closure, rank-mismatch impossibility, and rank equality. They do not add a new arithmetic-geometry formalization burden and do not alter the Lean endpoint claim. A later Lean pass may add bridge-object, local-countercase, local-bridge-exclusion, and local-discharge anchors without changing the present proof boundary.
